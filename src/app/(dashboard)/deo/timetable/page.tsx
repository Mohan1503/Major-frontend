...
import { Loader2, CheckCircle, Upload, Plus, Pencil, Calendar, Settings2, Clock, ArrowRight, ArrowLeft, List } from "lucide-react";
...
export default function DEOTimetablePage() {
    ...
    const [activeTab, setActiveTab] = useState<"create" | "view">(showViewTab ? "view" : (isEditMode ? "create" : "view"));
    const [step, setStep] = useState<"config" | "editor">(isEditMode ? "config" : "config");
    const [loading, setLoading] = useState(false);
    const [loadingTimetableData, setLoadingTimetableData] = useState(isEditMode);
    const [message, setMessage] = useState<{ type: 'success' | 'error', text: string } | null>(null);
    const [facultyList, setFacultyList] = useState<any[]>([]);

    // Configuration State
    const [config, setConfig] = useState({
        dept: isEditMode ? (searchParams.get("dept") || "CSE") : "CSE",
        section: isEditMode ? (searchParams.get("section") || "A") : "A",
        year: isEditMode ? parseInt(searchParams.get("year") || "2") : 2,
        semester: isEditMode ? parseInt(searchParams.get("semester") || "3") : 3,
    ...
    useEffect(() => {
        if (isEditMode && config.dept && config.section && config.semester) {
            setLoadingTimetableData(true);
            const loadTimetable = async () => {
                try {
                    const res = await api.get(`/timetable/view?dept=${config.dept}&section=${config.section}&semester=${config.semester}`);
                    
                    if (!res.data || res.data.length === 0) {
                        console.warn("No timetable data found for editing");
                        setLoadingTimetableData(false);
                        return;
                    }
                    
                    // Transform the response data to grid format
                    ...
                    // For each day, sort by start time and assign period numbers
                    DAYS.forEach(day => {
                        const daySlots = slotsByDay[day] || [];
                        daySlots.sort((a, b) => a.startTime.localeCompare(b.startTime));
                        
                        daySlots.forEach((slot, index) => {
                            const periodIndex = index + 1; // 1-indexed
                    ...
                    // Automatically move to editor step when timetable is loaded
                    setStep("editor");
                } catch (err) {
                    console.error("Failed to load timetable for editing", err);
                    setMessage({ type: 'error', text: "Failed to load timetable for editing" });
                } finally {
                    setLoadingTimetableData(false);
                }
            };
    ...
    }, [isEditMode, config.dept, config.section, config.semester]);
    ...
    const handlePublish = async () => {
        ...
        DAYS.forEach(day => {
            ...
            for (let p = 1; p <= 7; p++) {
                ...
                // Include the period regardless of whether it has data or not
                // The backend will decide if it's a break or not
                if (cell && (cell.subjectName || cell.subjectCode)) {
                    periodsData[p] = {
                        subjectName: cell.subjectName || "",
                        subjectCode: cell.subjectCode || "",
                        facultyEmail: cell.facultyEmail || ""
                    };
                }
                // If no cell data, backend will create a break
            }
        ...
        console.log("Publishing timetable with payload:", JSON.stringify(payload, null, 2));

        try {
            if (isEditMode) {
                // Update existing timetable
                await api.put("/deo/update-timetable", payload);
            ...
            setTimeout(() => {
                setStep("config");
                if (isEditMode) {
                    // Redirect back to all timetables view
                    window.location.href = "/deo/timetable?view=true";
                }
            }, 2000);
        } catch (err: any) {
            console.error("Error saving timetable:", err);
            const errorMsg = err.response?.data?.message || err.response?.data || err.message || "Failed to save timetable.";
    ...
    if (step === "config") {
        // Show loading state when in edit mode and loading timetable data
        if (loadingTimetableData && isEditMode) {
            return (
                <div className="flex h-[60vh] items-center justify-center flex-col gap-4">
                    <Loader2 className="h-12 w-12 animate-spin text-primary" />
                    <p className="text-lg text-muted-foreground font-medium">Loading timetable for editing...</p>
                </div>
            );
        }
        ...
        return (
            <div className="space-y-6">
                {/* Tab Switcher - Hide in edit mode */}
                {!isEditMode && (
            ...
            <motion.div key="config" variants={container} initial="hidden" animate="show" className="max-w-4xl mx-auto space-y-6">
                <motion.div variants={item} className="flex flex-col gap-2">
                    <h1 className="text-4xl font-black tracking-tight text-foreground">
                        {isEditMode ? "Edit Timetable" : "Create Timetable"}<span className="text-primary">.</span>
                    </h1>
                    <p className="text-lg text-muted-foreground">
                        {isEditMode ? "Modify the schedule for this section." : "Setup class scheduling and duration rules."}
                    </p>
                </motion.div>
                                ...
                                <div className="space-y-4">
                                    ...
                                    <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
                                        <div className="space-y-2">
                                            <Label>Dept</Label>
                                            <Input value={config.dept} onChange={e => setConfig({ ...config, dept: e.target.value })} disabled={isEditMode} className={`bg-muted/50 ${isEditMode ? 'cursor-not-allowed opacity-60' : ''}`} />
                                        </div>
                                        <div className="space-y-2">
                                            <Label>Section</Label>
                                            <Input value={config.section} onChange={e => setConfig({ ...config, section: e.target.value })} disabled={isEditMode} className={`bg-muted/50 ${isEditMode ? 'cursor-not-allowed opacity-60' : ''}`} />
                                        </div>
                                        <div className="space-y-2">
                                            <Label>Semester</Label>
                                            <Input type="number" value={config.semester} onChange={e => setConfig({ ...config, semester: parseInt(e.target.value) })} disabled={isEditMode} className={`bg-muted/50 ${isEditMode ? 'cursor-not-allowed opacity-60' : ''}`} />
                                        </div>
                                        <div className="space-y-2">
                                            <Label>Year</Label>
                                            <Input type="number" value={config.year} onChange={e => setConfig({ ...config, year: parseInt(e.target.value) })} disabled={isEditMode} className={`bg-muted/50 ${isEditMode ? 'cursor-not-allowed opacity-60' : ''}`} />
                                        </div>
                                    </div>
                                </div>
            ...
            <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
                ...
                <motion.div variants={item} className="flex gap-2">
                    ...
                    <Button onClick={handlePublish} disabled={loading} className="font-bold shadow-lg shadow-primary/20">
                        ...
                        {isEditMode ? "Update Timetable" : "Publish Timetable"}
                    </Button>
                </motion.div>
            </div>
