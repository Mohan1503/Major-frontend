...
import { Loader2, Calendar, Search, ArrowLeft, Building2, BookOpen, GraduationCap, Edit2, Trash2 } from "lucide-react";
...
export function AllTimetablesView({ userRole }: AllTimetablesViewProps) {
    ...
    const handleDelete = async () => {
        ...
        setDeleting(true);
        try {
            await api.delete(`/deo/delete-timetable`, {
                params: {
                    dept: selectedSection.dept,
    ...
    const handleEdit = () => {
        if (!selectedSection) return;
        // Redirect to DEO timetable page with edit mode
        const params = new URLSearchParams({
            edit: "true",
            dept: selectedSection.dept,
            section: selectedSection.section,
            semester: selectedSection.semester.toString(),
            year: selectedSection.year.toString()
        });
        window.location.href = `/deo/timetable?${params.toString()}`;
    };
        ...
        return (
            <motion.div initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }} className="space-y-6">
                <div className="flex items-center justify-between gap-4">
                    ...
                    {/* Edit and Delete Buttons - Only for DEO */}
                    {userRole === "DEO" && (
                        ...
                        <DialogHeader>
                            ...
                            <DialogDescription>
                                ...
                                <span className="font-semibold text-foreground">
                                    {selectedSection?.dept} - Section {selectedSection?.section} (Sem {selectedSection?.semester})
                                </span>
